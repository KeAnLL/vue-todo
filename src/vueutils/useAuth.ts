...
import { UndefinedUserEmailError, UndefinedUserPasswordError } from '@/utils/error';
...
const handleSignUp = async (credentials: Credentials) => {
    try {
        ...
        if(error instanceof UndefinedUserEmailError || error instanceof UndefinedUserPasswordError) {
            return /* Something here */;
        } else {
